#!/bin/bash
# Smoke test runner for legacy-ibmi-module-analyzer v0.1.1
...
echo -e "${BLUE}=== legacy-ibmi-module-analyzer v0.1.1 Smoke Test ===${NC}\n"
...
if codex exec -C . -s read-only --ephemeral -m gpt-5.4-mini "$POSITIVE_PROMPT" > /tmp/codex-result.txt 2>&1; then
  ...
  if grep -q "MODULE-AUTH-MODULE-001\|module-overview\|01-operation-flow" /tmp/codex-result.txt; then
    CODEX_STATUS="passed"
    echo -e "${GREEN}✓ Codex CLI: passed (output structure correct)${NC}"
  fi
else
...
if claude -p --model haiku --permission-mode dontAsk --tools Read --max-budget-usd 0.20 "$POSITIVE_PROMPT" > /tmp/claude-result.txt 2>&1; then
  ...
  if grep -q "MODULE-AUTH-MODULE-001\|module-overview\|01-operation-flow" /tmp/claude-result.txt; then
    CLAUDE_STATUS="passed"
    echo -e "${GREEN}✓ Claude Code: passed (output structure correct)${NC}"
  fi
else
...
if command -v opencode &> /dev/null; then
  if opencode run -m opencode/minimax-m2.5-free "$POSITIVE_PROMPT" > /tmp/opencode-result.txt 2>&1; then
    ...
    if grep -q "MODULE-AUTH-MODULE-001\|module-overview\|01-operation-flow" /tmp/opencode-result.txt; then
      OPENCODE_STATUS="passed"
      echo -e "${GREEN}✓ OpenCode: passed (output structure correct)${NC}"
    fi
  else
...
if [ "$CODEX_STATUS" = "passed" ] && [ "$CLAUDE_STATUS" = "passed" ] && [ "$OPENCODE_STATUS" = "passed" ]; then
  echo -e "${GREEN}✓ All tests PASSED - ready for v0.1.1 scorecard${NC}"
  exit 0
else
