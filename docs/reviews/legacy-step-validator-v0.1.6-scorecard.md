...
superseded_by: docs/reviews/legacy-step-validator-v0.1.7-scorecard.md
...
# Skill Review Scorecard: legacy-step-validator v0.1.6

## Change Under Review

v0.1.6 aligns validator checks with program-analyzer v0.2.4. Program-analysis
validation now checks that Routine Logic Details include routine-local
field-lineage/carrier rows and routine-local exception closure rows, with
Error Code Inventory links and TBDs for unresolved carrier or exception gaps.
...
A 9.0 cap applies because v0.1.6 checklist changes have not yet been
...
| --- | --- | --- | --- |
| STEP-VAL-060 | Medium | v0.1.6 routine-local lineage / exception validation has not been smoke-executed across the three target runtimes. | Run positive and blocked validations against analyzer v0.2.4 artifacts, including a blocked fixture where routine calculations lack carrier ties or exception closure rows. |
