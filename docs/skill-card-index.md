...
| Supplemental operations | [`legacy-ibmi-program-list-batch`](../skills/legacy-ibmi-program-list-batch/SKILL.md#skill-card) | Turn a program-list CSV/Excel export into Copilot Chat-friendly one-program prompts, status tracking, and resumable batch control. | Repo scan or inventory program list | Program analyzer per row; flow/program-set review after rows are classified | `program-batch-plan.md`, `program-list-status.csv`, prompt queue, batch manifest | Treating one Copilot Chat as a concurrent isolated batch runner. |
