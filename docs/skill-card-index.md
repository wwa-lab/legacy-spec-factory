...
| Module-first context intake | [`legacy-current-state-discovery`](../skills/legacy-current-state-discovery/SKILL.md#skill-card) | Extract current-state functional reports and catalogs from selected documents/RAG evidence. | Document intake, RAG retrieval, SME prompts, spreadsheets, or project folders | SME review, module context intake, BRD preparation, and IBM i analyzers for code-required gaps | Current-state discovery package | Treating high-level documents as behavior ground truth or hiding code-required gaps. |
...
| Layer 1 IBM i extraction | [`legacy-ibmi-program-analyzer`](../skills/legacy-ibmi-program-analyzer/SKILL.md#skill-card) | Analyze one RPGLE, CLLE, or COBOL program; can run standalone for output inspection without inventory. | Inventory for chain-ready output; none for standalone exploratory output | Flow analyzer, module analyzer, data model, batch digest, spec synthesis after chain-ready approval | `program-analysis-<OBJ-ID>.md` or exploratory `program-analysis.md` | Inferring business meaning from names alone or treating exploratory output as downstream-ready. |
| Supplemental operations | [`legacy-ibmi-program-list-batch`](../skills/legacy-ibmi-program-list-batch/SKILL.md#skill-card) | Turn a program-list CSV/Excel export into Copilot Chat-friendly one-program prompts, status tracking, and resumable batch control. | Repo scan or inventory program list | Program analyzer per row; later flow analyzer only for a selected flow/program set | `program-batch-plan.md`, `program-list-status.csv`, prompt queue, batch manifest | Treating one Copilot Chat as a concurrent isolated batch runner or forcing unrelated programs into one review. |
| Layer 1 IBM i synthesis control | [`legacy-ibmi-flow-analyzer`](../skills/legacy-ibmi-flow-analyzer/SKILL.md#skill-card) | Validate finalized reader-first program analyses, prepare a lossless source/fact pack, and let the executing skill LLM create one coverage-complete SME review. | Finalized program analyzer outputs; inventory only for targeted missing-program recovery | SME/Dify review only under v0.4.0; module/BRD/spec use requires a separately migrated compatibility contract | `<folder_slug>--sme-core-review.md` plus readiness/source-pack/fact/coverage controls | Omitting a per-program fact or presenting SME list order as a confirmed call chain. |
