...
| Skill | Status | Role |
|---|---|---|
| `legacy-modernization-orchestrator` | ✅ v0.2.2 | Route the user through the full e2e chain, including quality-aware scattered-document routing and owner-accepted sparse context before intake |
...
- `legacy-modernization-orchestrator` v0.2.2 routes the full chain
  (document triage/flow normalization → risk acceptance when needed → context
  intake, or inventory → program → flow → module → spec).
