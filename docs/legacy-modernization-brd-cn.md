...
我们要做的是：从 IBM i / AS400 旧系统代码和相关运行证据中，生成一份**有证据链、可追溯、可审阅、可由 SME 和业务方确认的 BRD 草稿**。
...
- `legacy-ibmi-module-analyzer`：合成 module 级别的四视图理解；
...
- program analysis 和 flow analysis 的作用是提供证据；
- module analysis 负责形成业务边界和四视图理解；
- BRD writer 根据一个明确的 capability 生成业务可读的 BRD Package。
...
可选章节包括安全 / 认证要求、补充流程或设计说明、source document mapping。可选章节只有在证据或 SME 输入支持时才写，不能为了填模板而编造。
...
代码中无法确认的信息不能写成事实，必须进入 Open Questions。
...
如果一个 flow 依赖某个会改变状态的 routine，而这个 routine 只是 `indexed_only`，那么不能把它写成确定业务事实。要么回到 program analysis 做 deep-read，要么记录 SME waiver 或 open question。
...
4. **流程分析**：多个 program 之间的 transaction flow、data lineage、persistence、exception chain 被连接；
5. **module synthesis**：四视图 module understanding 形成 capability 和 rule seed；
6. **BRD traceability**：BRD 里的行为、规则、验证场景、开放问题都链接到证据；
