...
For `daily_delivery` mode, this report must distinguish blockers from delivery
warnings. Non-critical gaps may travel to the daily-delivery review pack, but
the BRD remains non-approved and cannot feed spec writing, SDD handoff, audit
baseline, or knowledge publication.
...
- **Code-Backed Artifacts:** object map present? yes/no; program analyses X/Y;
  flow analyses A/B
- **Daily Delivery Findings:** blocking exceptions listed? yes/no; delivery
  warnings linked to review pack? yes/no
