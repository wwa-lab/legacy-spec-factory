...
| 0d — Flow Context Normalization | Convert or triage scattered documents before module context intake | `legacy-flow-context-normalizer` | Implemented v0.1.5 | Use when Visio, Word, Excel, PDF, PowerPoint, exported diagrams, RAG summaries, or SME notes exist but the standard flows are not yet normalized or SME-reviewed. Strong/partial inputs produce Mermaid-backed draft flows; sparse authorized inputs produce `triage_needs_source_enrichment`; owner-accepted sparse packages may become `ready_with_warnings` with carry-forward TBDs; only unauthorized, unreadable, out-of-scope, or boundaryless inputs block. Draft flow steps are never promoted to approved rules. |
| 0d — Risk-Accepted Sparse Context | Continue after source owner/SME confirms no additional flow input can be provided | `legacy-module-context-intake` | Implemented v0.1.2 | Use only when `flow-context-index.yaml` has `normalization.status: ready_with_warnings`, `quality_level: L1 sparse`, and `risk_acceptance.status: accepted`. Context intake must preserve all gaps as low-confidence TBDs and must not route directly to module analysis or BRD generation without its own gates. |
