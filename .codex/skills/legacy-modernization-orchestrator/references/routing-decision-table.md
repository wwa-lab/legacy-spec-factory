...
| 0n — Module Context Ready | Synthesize context-only module draft | `legacy-ibmi-module-analyzer` | Implemented v0.2.0 | Use only when the user explicitly accepts a context-only path for this cycle or no source/object evidence is available. Preserve open questions as TBDs and do not claim code-backed approval. |
...
| 2c — Inventory Done | Understand program logic | `legacy-ibmi-program-analyzer` | **Implemented v0.2.0** | Run per program; produces `program-analysis-<OBJ-ID>.md` with logic decomposition, key fields, field-level File I/O, and exception closure |
...
| 3a — Program Analysis In Progress | Continue | `legacy-ibmi-program-analyzer` | **Implemented v0.2.0** | Complete coverage before routing to flow-analyzer, including v0.2 key-field / mutation / exception ledgers |
| 3b — Program Analysis Done | Analyze a call chain | `legacy-ibmi-flow-analyzer` | **Implemented v0.2.0** | Required when business event spans multiple programs; supports 7 trigger models plus replay, lineage, persistence, and exception-chain evidence |
| 3c — Flow Analysis In Progress | Continue | `legacy-ibmi-flow-analyzer` | **Implemented v0.2.0** | Complete all in-scope flows before module synthesis, including replay / lineage / persistence / exception-chain coverage |
| 3d — Flow Analysis Done | Synthesize the module | `legacy-ibmi-module-analyzer` | **Implemented v0.2.0** | Produces the canonical 4-view module analysis plus readiness, critical field, persistence, and exception summaries under `04_modules/` per `docs/module-analysis-model.md` |
| 3e — Module Analysis In Progress | Continue | `legacy-ibmi-module-analyzer` | **Implemented v0.2.0** | All four views must reach `approved` or `approved_with_non_blocking_tbd`; module readiness and BRD crosswalk must carry any gaps |
...
| 3f — Approved BRD Package plus explicit post-BRD promotion / disposition decision | Produce capability spec | `legacy-spec-writer` | **Implemented v0.1.4** | One spec per promoted `CAP-*`; consumes approved BRD Package plus analyzer v0.2 replay / lineage / persistence / exception evidence |
