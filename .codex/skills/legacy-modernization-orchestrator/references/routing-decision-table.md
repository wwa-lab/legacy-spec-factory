...
| 0n — Module Context Ready | Synthesize context-only module draft | `legacy-ibmi-module-analyzer` | Implemented v0.2.1 | Use only when the user explicitly accepts a context-only path for this cycle or no source/object evidence is available. Preserve open questions as TBDs and do not claim code-backed approval. |
...
| 2c — Inventory Done | Understand program logic | `legacy-ibmi-program-analyzer` | **Implemented v0.2.1** | Run per program; produces `program-analysis-<OBJ-ID>.md` with Call Evidence, source identifier + meaning fields, File I/O Purpose, dynamic-call resolution, Error Code Inventory, and exception closure |
...
| 3a — Program Analysis In Progress | Continue | `legacy-ibmi-program-analyzer` | **Implemented v0.2.1** | Complete coverage before routing to flow-analyzer, including Call Evidence, key-field meaning, File I/O Purpose, dynamic-call resolution, Error Code Inventory, and exception ledgers |
| 3b — Program Analysis Done | Analyze a call chain | `legacy-ibmi-flow-analyzer` | **Implemented v0.2.1** | Required when business event spans multiple programs; supports 7 trigger models plus replay, edge Evidence Source / Resolution, lineage, persistence purpose, and exception-chain evidence |
| 3c — Flow Analysis In Progress | Continue | `legacy-ibmi-flow-analyzer` | **Implemented v0.2.1** | Complete all in-scope flows before module synthesis, including replay / edge resolution / lineage / persistence / exception-chain coverage |
| 3d — Flow Analysis Done | Synthesize the module | `legacy-ibmi-module-analyzer` | **Implemented v0.2.1** | Produces the canonical 4-view module analysis plus readiness, edge-resolution, critical field, persistence purpose, and exception summaries under `04_modules/` per `docs/module-analysis-model.md` |
| 3e — Module Analysis In Progress | Continue | `legacy-ibmi-module-analyzer` | **Implemented v0.2.1** | All four views must reach `approved` or `approved_with_non_blocking_tbd`; module readiness and BRD crosswalk must carry edge-resolution, data, exception, and gap coverage |
| 3f — Module Analysis Done, no approved BRD Package | Produce legacy BRD for SME / business discovery review | `legacy-brd-writer` | **Implemented v0.1.7** | One legacy-system-only BRD Package per selected `CAP-*`; sections 1-9 are required for review, 10-12 optional/evidence-backed. For standard BRDs, module analysis must be backed by `01_inventory/object-map.md`, required program analyses, and required flow analyses. |
...
| 3f — Approved BRD Package plus explicit post-BRD promotion / disposition decision | Produce capability spec | `legacy-spec-writer` | **Implemented v0.1.5** | One spec per promoted `CAP-*`; consumes approved BRD Package plus analyzer v0.2.1 replay / lineage / persistence / error-inventory / exception evidence |
...
acceptance and must remain non-approved.
