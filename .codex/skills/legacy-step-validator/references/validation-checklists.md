...
| Entry points, control flow, call graph (tree + call-site table + reverse index), object dependencies, file I/O, external calls, error handling, TBDs, SME checklist sections present | 3 | blocking |
...
| Flow-header (if present) reconciled against code-derived call graph; mismatches recorded as TBDs | 9 | blocking |
...
| Check | Maps to dimension | Severity |
| --- | --- | --- |
| File exists with all nine sections populated | 3 | blocking |
...
| Check | Maps to dimension | Severity |
| --- | --- | --- |
| Calls match the upstream program-analyses' External Calls sections | 4 | blocking |
