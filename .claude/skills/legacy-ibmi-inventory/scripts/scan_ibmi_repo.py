...
def detect_source_revision(root: Path, ignore_paths: list[Path] | None = None) -> dict[str, Any]:
    ...
    if top_level.returncode != 0:
        ...
        return {
            ...
            "freshness_note": "No Git metadata was available; downstream reuse cannot prove freshness.",
        }
    ...
    return {
        ...
        "key": f"git:{full_head}",
        "freshness_note": (
            "Stable reuse key; downstream cache is fresh only when the same clean HEAD is observed."
            if dirty is False
            else "Source worktree had uncommitted changes; downstream cache should be refreshed before reuse."
        ),
    }
...
def build_summary(
    ...
    source_revision = detect_source_revision(root, ignore_paths=[out_dir] if out_dir else None)
    return {
        "schema_version": "0.1",
        ...
        "source_revision_key": source_revision["key"],
...
def write_outputs(members: list[SourceMember], root: Path, out_dir: Path, large_threshold: int) -> list[Path]:
    ...
    scan_summary.write_text(to_yaml(build_summary(root, members, large_threshold, out_dir)) + "\n", encoding="utf-8")
